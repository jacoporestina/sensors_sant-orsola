...
data_files = [
    "data/fruit_measurements.csv",   # Existing file
    "data/fruit_analysis.csv"        # Add your new file here
]
...
for file_path in data_files:
    ...
    # Output folder (separate for each file)
    output_dir = f"statistics_output/"
    os.makedirs(output_dir, exist_ok=True)

    # Identify variable columns (exclude metadata)
    excluded = ['date', 'date_str', 'treatment', 'tunnel']
    variables = [col for col in df.columns if col not in excluded]

    # Loop through each variable
    for variable in variables:
        var_folder = os.path.join(output_dir, variable.replace(' ', '_').replace('/', '_'))
        os.makedirs(var_folder, exist_ok=True)

        result_txt = os.path.join(var_folder, f"{variable.replace(' ', '_').replace('/', '_')}_anova_results.txt")
        with open(result_txt, 'w') as summary_file:
            summary_file.write(f"ANOVA Results and Assumption Tests for {variable}\n\n")
            ...
            for date in df['date_str'].unique():
                ...
                try:
                    ...
                    plt.tight_layout()
                    plt.savefig(os.path.join(var_folder, f"{date}_qq_plot.png"))
                    plt.close()
